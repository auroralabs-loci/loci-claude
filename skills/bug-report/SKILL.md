...
for collection (they may be the thing that's broken). Use only: Read, Bash,
Glob, Grep. Check 1 *records* whether MCP tools are visible — it does not
*invoke* them.
...
11. **MCP config** — Read `<plugin-dir>/.mcp.json`. Record the configured
    server URL. If missing, record "MISSING".
...
| 13 | Quota not exceeded | If check 1 passed, call `mcp__plugin_loci_loci__loci_usage_for_period` with `from_ts` = 24h ago, `to_ts` = now (UTC ISO). Compare `total_tokens` against tier limit (free=30000, premium=300000, enterprise=1500000). Plan from MCP response or default to free. | `total_tokens < tier_limit` |
...
If check 1 failed (MCP not visible), check 13 automatically FAILs with
"MCP not connected — cannot check quota".

Check 13 is the only check that *invokes* an MCP tool. Skip it if check 1
failed; record "skipped: MCP not connected" in the detail column.
...
3. **MCP response** — did the MCP call return timing/energy data? Common
   ...
   empty response. Confirm `.mcp.json` points at `https://mcp.auroralabs.com/mcp/v1`
   (the URL was relocated from `loci.auroralabs.com` in 0.1.66; an outdated
   plugin install may still reference the old host).
