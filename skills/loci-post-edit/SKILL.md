...
```
# Whole JSON to a file you control — a path inside the project. NEVER /tmp or any out-of-project path (triggers a Claude Code permission prompt that halts automation):
...
NEVER write to `/tmp/`, `/var/tmp/`, or any path outside the working directory
(on Windows, `/tmp/...` additionally cannot be resolved by the venv Python):
Claude Code prompts the user for permission on every out-of-project access,
halting automated preflight/post-edit/eval runs. Always write to a path inside
the working directory (e.g. `.loci-build/`) so every tool — bash, jq, Python,
MCP — sees the same file.
...
Hot-path blocks that end in `bl` / `blx` are *call sites*; the MCP
...
2. **Performance** — fires when MCP timing returned. Captures
...
3. **Energy** — fires when MCP returned energy. Status logic same as
