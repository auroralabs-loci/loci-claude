"""The one-time welcome banner must survive plugin upgrades.

The ``.welcome-shown`` marker lives at ``${HOME}/.loci/.welcome-shown``, shared
across versions (falling back to PLUGIN_DIR only if ``~/.loci`` is unwritable).
If it lived inside the versioned plugin cache dir instead, every upgrade would
land in a fresh dir with no marker and re-show the banner.

Strategy: run session-init.sh from plugin-A (banner shows, marker written),
then from plugin-B simulating the upgrade (banner must NOT re-appear).
"""
...
def _stage_plugin_dir(parent: Path, name: str, version: str) -> Path:
    ...
    # lib/ carries the shell entry points session-init.sh sources and runs.
...
def test_welcome_survives_plugin_version_upgrade(tmp_path):
    """First session in plugin-A shows the welcome; a second session in
    plugin-B (a version bump in a fresh cache dir) must NOT show it again."""
    home = tmp_path / "home"
    home.mkdir()
