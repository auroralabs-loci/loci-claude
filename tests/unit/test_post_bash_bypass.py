"""The PostToolUse(Bash) hook that names a source edited outside the snapshot.

`hooks/post-bash-bypass.sh` is deliberately thin, same shape as
`manifest-status-nudge.sh`: it is silent when `loci` is
missing, and otherwise pipes stdin straight into `loci hook post-bash`,
which does the actual JSON parsing and the file-state check. So this test fakes `loci`
itself rather than any state on disk.

Two things are load-bearing:
* **It must never exit 2.** On `PostToolUse` a nonzero/blocking exit
...
HOOK = PLUGIN_ROOT / "hooks" / "post-bash-bypass.sh"
...
def test_absent_loci_is_silent_this_fires_on_every_bash_call(tmp_path):
...
def test_the_verbs_output_reaches_the_harness_unmangled(tmp_path):
    doc = {"hookSpecificOutput": {"hookEventName": "PostToolUse",
                                  "additionalContext": "LOCI: src/f.c changed outside the Edit/Write tools"}}
...
# ── the loop hazard: on PostToolUse a blocking exit is worse than Stop ─
